import { n as useServerFn } from "./createSsrRpc-DfY1OTDB.js";
import { i as listNotes, n as deleteNote, t as createAndAnalyzeNote } from "./notes.functions-Dh-vkZkJ.js";
import { useState } from "react";
import { Link, Outlet, useNavigate } from "@tanstack/react-router";
import { jsx, jsxs } from "react/jsx-runtime";
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import { toast } from "sonner";
import { BookOpen, FileText, Plus, Sparkles, Trash2 } from "lucide-react";
//#region src/routes/_authenticated/notes.tsx?tsr-split=component
function NotesLayout() {
	const qc = useQueryClient();
	const navigate = useNavigate();
	const listFn = useServerFn(listNotes);
	const createFn = useServerFn(createAndAnalyzeNote);
	const deleteFn = useServerFn(deleteNote);
	const notesQ = useQuery({
		queryKey: ["notes"],
		queryFn: () => listFn()
	});
	const [composerOpen, setComposerOpen] = useState(false);
	const [title, setTitle] = useState("");
	const [text, setText] = useState("");
	const create = useMutation({
		mutationFn: () => createFn({ data: {
			title: title || void 0,
			sourceText: text
		} }),
		onSuccess: (n) => {
			toast.success("Notes processed!");
			setTitle("");
			setText("");
			setComposerOpen(false);
			qc.invalidateQueries({ queryKey: ["notes"] });
			navigate({
				to: "/notes/$noteId",
				params: { noteId: n.id }
			});
		},
		onError: (e) => toast.error(e instanceof Error ? e.message : "Could not analyze notes")
	});
	const del = useMutation({
		mutationFn: (id) => deleteFn({ data: { id } }),
		onSuccess: () => qc.invalidateQueries({ queryKey: ["notes"] })
	});
	const onFile = async (file) => {
		if (!file.type.startsWith("text/") && !file.name.endsWith(".md") && !file.name.endsWith(".txt")) {
			toast.error("Please upload a plain text or markdown file (or paste your notes).");
			return;
		}
		setText((await file.text()).slice(0, 4e4));
		if (!title) setTitle(file.name.replace(/\.[^.]+$/, ""));
		setComposerOpen(true);
	};
	return /* @__PURE__ */ jsxs("div", {
		className: "mx-auto max-w-6xl px-4 py-8 sm:px-6 md:px-8 md:py-12",
		children: [
			/* @__PURE__ */ jsxs("header", {
				className: "grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 sm:flex sm:flex-wrap sm:justify-between",
				children: [/* @__PURE__ */ jsxs("div", {
					className: "min-w-0",
					children: [/* @__PURE__ */ jsx("h1", {
						className: "font-display truncate text-3xl font-bold tracking-tight md:text-4xl",
						children: "Smart Notes"
					}), /* @__PURE__ */ jsx("p", {
						className: "mt-1 text-sm text-muted-foreground",
						children: "Paste notes or upload a file — get a summary, key points, and flashcards."
					})]
				}), /* @__PURE__ */ jsxs("button", {
					onClick: () => setComposerOpen((s) => !s),
					className: "bg-gradient-brand inline-flex shrink-0 items-center gap-1.5 rounded-full px-4 py-2 text-sm font-semibold text-primary-foreground shadow-glow",
					children: [/* @__PURE__ */ jsx(Plus, { className: "h-4 w-4" }), " New notes"]
				})]
			}),
			composerOpen && /* @__PURE__ */ jsxs("div", {
				className: "shadow-card mt-6 overflow-hidden rounded-2xl border border-border bg-card",
				children: [
					/* @__PURE__ */ jsx("div", {
						className: "border-b border-border bg-muted/30 p-4",
						children: /* @__PURE__ */ jsx("input", {
							value: title,
							onChange: (e) => setTitle(e.target.value),
							placeholder: "Title (optional) — e.g. Bio Ch.4 Photosynthesis",
							className: "w-full bg-transparent text-lg font-semibold outline-none placeholder:text-muted-foreground"
						})
					}),
					/* @__PURE__ */ jsx("textarea", {
						value: text,
						onChange: (e) => setText(e.target.value),
						placeholder: "Paste your notes, lecture transcript, or chapter here…",
						className: "h-56 w-full resize-y bg-card p-4 text-sm outline-none"
					}),
					/* @__PURE__ */ jsxs("div", {
						className: "flex flex-wrap items-center justify-between gap-3 border-t border-border p-3",
						children: [/* @__PURE__ */ jsxs("label", {
							className: "inline-flex cursor-pointer items-center gap-2 rounded-full border border-border bg-background px-3 py-1.5 text-xs font-semibold hover:bg-accent",
							children: [
								/* @__PURE__ */ jsx(FileText, { className: "h-3.5 w-3.5" }),
								" Upload .txt / .md",
								/* @__PURE__ */ jsx("input", {
									type: "file",
									accept: ".txt,.md,text/*",
									className: "hidden",
									onChange: (e) => {
										const f = e.target.files?.[0];
										if (f) onFile(f);
									}
								})
							]
						}), /* @__PURE__ */ jsxs("div", {
							className: "flex items-center gap-2",
							children: [/* @__PURE__ */ jsxs("span", {
								className: "text-xs text-muted-foreground",
								children: [text.length.toLocaleString(), " chars"]
							}), /* @__PURE__ */ jsxs("button", {
								onClick: () => create.mutate(),
								disabled: create.isPending || text.length < 20,
								className: "bg-gradient-brand inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold text-primary-foreground shadow-glow disabled:opacity-60",
								children: [/* @__PURE__ */ jsx(Sparkles, { className: "h-4 w-4" }), create.isPending ? "Analyzing…" : "Analyze with AI"]
							})]
						})]
					})
				]
			}),
			/* @__PURE__ */ jsx("section", {
				className: "mt-8",
				children: notesQ.isLoading ? /* @__PURE__ */ jsx("p", {
					className: "text-muted-foreground",
					children: "Loading…"
				}) : notesQ.data && notesQ.data.length > 0 ? /* @__PURE__ */ jsx("ul", {
					className: "grid gap-3 sm:grid-cols-2 lg:grid-cols-3",
					children: notesQ.data.map((n) => /* @__PURE__ */ jsxs("li", {
						className: "group relative",
						children: [/* @__PURE__ */ jsxs(Link, {
							to: "/notes/$noteId",
							params: { noteId: n.id },
							className: "shadow-card block h-full rounded-2xl border border-border bg-card p-5 transition-transform hover:-translate-y-1",
							children: [
								/* @__PURE__ */ jsx("div", {
									className: "bg-gradient-brand inline-flex h-9 w-9 items-center justify-center rounded-xl shadow-glow",
									children: /* @__PURE__ */ jsx(BookOpen, { className: "h-4.5 w-4.5 text-primary-foreground" })
								}),
								/* @__PURE__ */ jsx("h3", {
									className: "mt-3 line-clamp-2 font-semibold",
									children: n.title
								}),
								n.summary && /* @__PURE__ */ jsx("p", {
									className: "mt-2 line-clamp-3 text-sm text-muted-foreground",
									children: n.summary
								})
							]
						}), /* @__PURE__ */ jsx("button", {
							onClick: () => {
								if (confirm("Delete these notes?")) del.mutate(n.id);
							},
							className: "absolute right-3 top-3 rounded-md bg-background/80 p-1.5 opacity-0 backdrop-blur transition-opacity hover:bg-destructive hover:text-destructive-foreground group-hover:opacity-100",
							"aria-label": "Delete",
							children: /* @__PURE__ */ jsx(Trash2, { className: "h-3.5 w-3.5" })
						})]
					}, n.id))
				}) : /* @__PURE__ */ jsxs("div", {
					className: "rounded-2xl border border-dashed border-border bg-muted/30 p-12 text-center",
					children: [
						/* @__PURE__ */ jsx("div", {
							className: "bg-gradient-brand mx-auto grid h-12 w-12 place-items-center rounded-2xl shadow-glow",
							children: /* @__PURE__ */ jsx(BookOpen, { className: "h-6 w-6 text-primary-foreground" })
						}),
						/* @__PURE__ */ jsx("h2", {
							className: "font-display mt-4 text-xl font-bold",
							children: "No notes yet"
						}),
						/* @__PURE__ */ jsx("p", {
							className: "mt-1 text-sm text-muted-foreground",
							children: "Paste your first batch of notes and let AI do the heavy lifting."
						}),
						/* @__PURE__ */ jsxs("button", {
							onClick: () => setComposerOpen(true),
							className: "bg-gradient-brand mt-5 inline-flex items-center gap-2 rounded-full px-5 py-2 text-sm font-semibold text-primary-foreground shadow-glow",
							children: [/* @__PURE__ */ jsx(Plus, { className: "h-4 w-4" }), " Add notes"]
						})
					]
				})
			}),
			/* @__PURE__ */ jsx(Outlet, {})
		]
	});
}
//#endregion
export { NotesLayout as component };
